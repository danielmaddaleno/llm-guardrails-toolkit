...
`ToxicityDetector` has the same trap in reverse. Matching the word itself (`suicid`, `racist`) blocks "What is the number for the national suicide prevention hotline?" along with news copy and complaints about the same subjects, and as an output guard that turns a crisis line in the model's answer into an empty response. Its patterns therefore want a request for the content or a statement of intent ("write a racist joke", "how to commit suicide"), so harmful text phrased as description gets through. Keyword matching cannot separate the topic from the intent; use it for the obvious cases and put a classifier behind it. On the output side, where a false positive costs the user the whole answer, `ToxicityDetector(severity="warn")` records the hit and passes the text on, which is how `examples/bedrock_example.py` wires it.
